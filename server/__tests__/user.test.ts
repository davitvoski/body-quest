...
const addProfilePictureToAzureMock =
  addProfilePictureToAzure as jest.MockedFunction<
    typeof addProfilePictureToAzure
  >;
...
describe("Testing User Routes - PATCH", () => {
  test("returns 200, user updated", async () => {
    jest
      ...
      .mockImplementation(
        async (newUsername: string, newImage: string, _: string) => {
    ...
    const res = await request(app)
      ...
      .set("Cookie", [
        `connect.sid=${encodeURIComponent(JSON.stringify(mockSession))}`,
      ])
